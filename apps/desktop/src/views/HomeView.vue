...
const steps: HomeStep[] = [
  {
    ...
    shortTitle: "激活当前设备",
    title: "激活当前设备",
    copy: "输入卡密，生成当前设备可用的授权状态。",
    detail: "激活后会生成当前设备可用的 license_token，后续任务执行都基于这个状态。",
    status: "先完成激活",
    href: "/activate",
    action: "去激活 →",
  },
  {
    ...
    title: "输入链接，创建任务",
    copy: "任务创建后会立即通知插件执行。",
    detail: "支持淘宝、天猫链接。创建成功后，插件会接手执行，无需额外绑定。",
    status: "提交待提取链接",
    href: "/extract",
    action: "创建任务 →",
  },
...
const notes = [
  { icon: "📌", text: "建议顺序是先激活，再创建任务，再查看历史结果。" },
  { icon: "🔍", text: "如遇异常，可在帮助页查看排查建议。" },
