...
async def create_task(request: Request, authorization: Optional[str] = Header(None)):
    ...
    return await store.create_task(input_data, authorization)

@router.get("")
async def list_tasks(authorization: Optional[str] = Header(None)):
    items = await store.list_tasks(authorization)
...
async def get_task(task_id: str, authorization: Optional[str] = Header(None)):
    return await store.get_task(task_id, authorization)
...
async def request_archive(task_id: str, request: Request, authorization: Optional[str] = Header(None)):
    try:
        input_data = await request.json()
    except:
        input_data = {}
    return await store.request_archive(task_id, input_data, authorization)

@router.get("/{task_id}/archive")
async def get_archive(task_id: str, authorization: Optional[str] = Header(None)):
    return await store.get_archive(task_id, authorization)

@router.post("/{task_id}/convert")
async def convert_task(task_id: str, request: Request, authorization: Optional[str] = Header(None)):
    input_data = await request.json()
    return await store.convert_task(task_id, input_data, authorization)
